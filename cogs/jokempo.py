...
class Jokenpo(commands.Cog):
    ...
    async def jokenpo(self, ctx, modo: typing.Union[discord.Member, str] = ""):
        if len(ctx.message.mentions) == 0:
            ...
            embed_servidor.set_author(name="Jokenpo", icon_url="https://lh3.googleusercontent.com/cDo7huS5WTjjioOU2ds9t_KBwZ8wz9ttrlKfaafvBuzTkLAIM0jZNbVGXvL4QraXgefS=s150-rw")
            #embed_servidor.set_thumbnail(url="https://lh3.googleusercontent.com/cDo7huS5WTjjioOU2ds9t_KBwZ8wz9ttrlKfaafvBuzTkLAIM0jZNbVGXvL4QraXgefS=s150-rw")
            ...
            #embed_servidor.title = "Aceitar desafio"
            #embed_servidor.description = "```✅ para aceitar o desafio\n🚪 para recusar o desafio```"
            #embed_servidor.add_field(name="Aceitar desafio", value="```✅ para aceitar o desafio\n🚪 para recusar o desafio```", inline=False)
            ...
            try:
                ...
                if emoji == "✅":
                    ...
                    #embed_jogo.title = "Sua jogada"
                    #embed_jogo.description = "✊ - Pedra\n✋ - Papel\n✌️ - Tesoura"
                    #embed_jogo.add_field(name="Sua jogada", value="```✊ - Pedra\n✋ - Papel\n✌️ - Tesoura```")
                    try:
                    ...
                    #embed_servidor.title = "Desafio aceito"
                    #embed_servidor.description = "Aguardando jogadas"
                    #embed_servidor.set_field_at(0, name="Desafio aceito", value="```Aguardando jogadas```")
                    ...
                    try:
                        ...
                        if emoji_p1 == None:
                            ...
                            #embed_servidor.description = f"{ctx.author.name} não escolheu uma jogada e perdeu de WO"
                            ...
                            #embed_servidor.description = f"{modo.name} não escolheu uma jogada e perdeu de WO"
                            ...
                            #embed_servidor.description = f"Ambos jogadores jogaram {emoji_p1}"
                            ...
                            #embed_servidor.description = f"{emoji_p1} vs {emoji_p2}"
                            embed_servidor.add_field(name=f"{ctx.author.name} jogou:", value=f"{emoji_p1}", inline=False)
                            embed_servidor.add_field(name=f"{modo.name} jogou:", value=f"{emoji_p2}", inline=False)
                        else:
                            embed_servidor.title = f"{modo.name} venceu!"
                            #embed_servidor.description = f"{emoji_p2} vs {emoji_p1}"
                            embed_servidor.add_field(name=f"{ctx.author.name} jogou:", value=f"{emoji_p1}", inline=False)
                            embed_servidor.add_field(name=f"{modo.name} jogou:", value=f"{emoji_p2}", inline=False)
